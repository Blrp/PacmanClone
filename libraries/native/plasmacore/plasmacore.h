...
#define PLASMACORE_DISPATCH_EVENT(event_name_obj) \
  { \
    SLAG_FIND_TYPE( type_event_mgr, "EventManager" ); \
    SLAG_PUSH_REF( type_event_mgr->singleton() ); \
    SLAG_PUSH_REF( event_name_obj ); \
    SLAG_CALL( type_event_mgr, "on_native_event(String)" ); \
  }
...
struct Plasmacore
{
  ...
  // These are references to permanent literal string objects and
  // won't change during GC.
  SlagObject* event_launch;
  SlagObject* event_update;
  SlagObject* event_draw;
  SlagObject* event_key_press;
  SlagObject* event_key_release;
  SlagObject* event_mouse_move_relative;
  SlagObject* event_mouse_move_absolute;
  SlagObject* event_mouse_button_press;
  SlagObject* event_mouse_button_release;
  SlagObject* event_mouse_wheel;
  SlagObject* event_garbage_collected;
  SlagObject* event_resources_loaded;
