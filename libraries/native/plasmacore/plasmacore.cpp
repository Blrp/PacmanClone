      ...
      if (filename_len <= cur_len)
      {
        ...
        for (int i=0; i<=diff; i++)
        {
              ...
              && 0 == strncmp(filename,current_filename+i,filename_len))
          {
            ...
            if ( !is_id(prev_char) && (!is_id(next_char) || next_char=='.'))
            {
              // found our match
...
void plasmacore_launch()
{
  ...
  plasmacore.event_key_press = plasmacore_find_event_key( "key_press" );
  plasmacore.event_key_release = plasmacore_find_event_key( "key_release" );
  plasmacore.event_mouse_move_relative = plasmacore_find_event_key( "mouse_move_relative" );
  plasmacore.event_mouse_move_absolute = plasmacore_find_event_key( "mouse_move_absolute" );
  plasmacore.event_mouse_button_press = plasmacore_find_event_key(  "mouse_button_press" );
  plasmacore.event_mouse_button_release = plasmacore_find_event_key( "mouse_button_release" );
  plasmacore.event_mouse_wheel = plasmacore_find_event_key( "mouse_wheel" );
  plasmacore.event_garbage_collected = plasmacore_find_event_key( "garbage_collected" );
  plasmacore.event_resources_loaded = plasmacore_find_event_key( "resources_loaded" );
  plasmacore.event_textures_lost = plasmacore_find_event_key( "textures_lost" );

  SLAG_FIND_TYPE( type_event_mgr, "EventManager" );
  SlagObject* event_mgr = type_event_mgr->singleton();
  SLAG_SET_REF( event_mgr, "param_screen", *slag_main_object );
  SLAG_SET_REAL64( event_mgr, "param_x", plasmacore.display_width );
  SLAG_SET_REAL64( event_mgr, "param_y", plasmacore.display_height );
  PLASMACORE_DISPATCH_EVENT( plasmacore.event_launch );
}
...
bool plasmacore_update()
{
  ...
  while (first || plasmacore.time_debt >= 1.0 / plasmacore.updates_per_second)
  {
    ...
    SLAG_CALL( type_event_mgr, "update_system_services()" );

    if (SLAG_POP_LOGICAL() > 0)
    {
      draw_allowed = false;
    }
    else
    {
      PLASMACORE_DISPATCH_EVENT( plasmacore.event_update );
    }
  }
...
void plasmacore_draw()
{
  ...
  PLASMACORE_DISPATCH_EVENT( plasmacore.event_draw );
...
SlagObject* plasmacore_find_event_key( const char* event_type )
{
  ...
  SLAG_CALL( type_event_mgr, "find_event_key(String)" );
  return SLAG_POP_REF();
}
...
void plasmacore_hook_native_methods()
{
  ...
  slag_hook_native( "NativeSound","init(String,Logical)",
      NativeSound__init__String_Logical );
