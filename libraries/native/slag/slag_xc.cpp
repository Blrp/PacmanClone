...
SlagObject* FileError__init__String( SXCRef context, SXCRef filename );
SlagObject* FileNotFoundError__init__String( SXCRef context, SXCRef filename );
...
SlagLogical sxc_cast_to_logical( SlagReal64 n )
{
  return (n > 0) ? 1 : ((n < 0) ? -1 : 0);
}

SlagLogical sxc_cast_to_logical( SlagInt64 n )
{
  return (n > 0) ? 1 : ((n < 0) ? -1 : 0);
}

SlagLogical sxc_cast_to_logical( SlagInt32 n )
{
  return (n > 0) ? 1 : ((n < 0) ? -1 : 0);
}
